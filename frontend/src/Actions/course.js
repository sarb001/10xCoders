...
export const AllCourses = () => async(dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/allcourse`,{
            headers : {
...
export const MyAllCourses = () => async(dispatch) => {
    try {
        ...
        const { data } = await axios.get(` ${import.meta.env.VITE_BACKEND_URL}/api/v1/mycourses`,{
            headers : {
                'Content-Type' :  'application/json'
            }
        })
...
export const CourseLectures = (id) => async(dispatch) => {
    try {
        ...
        const {data} = await axios.get(` ${import.meta.env.VITE_BACKEND_URL}/api/v1/course/${id}`,{
            headers : {
                'Content-Type' : 'application/json'
            }
        })
...
export const DeleteMyCourse = (id) => async(dispatch) => {
    try {
        ...
        const { data } = await axios.delete(` ${import.meta.env.VITE_BACKEND_URL}/api/v1/course/${id}`,{
            headers: {
                'Content-Type' : 'application/json'
            }
        })
...
export const AddMyLecture = (id,formdata) => async(dispatch) => {
    try {
        ...
        const { data } = await axios.post(` ${import.meta.env.VITE_BACKEND_URL}/api/v1/course/${id}`,
            formdata,{
            headers: {
                'Content-Type' : 'multipart/form-data',
            },
...
export const DeleteMyLecture = (courseId,lectureId) => async(dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/v1/lecture?courseId=${courseId}&lectureId=${lectureId}`,
        {
            headers: {
                'Content-Type' : 'application/json'
            }
