...
export const Logout = () => async(dispatch) => {
    try {
        ...
        const data = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/loggingout`);
...
export const LoadUser = () => async(dispatch) => {
    try {
        ...
        const {data} = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/me`, {
            headers:{
                'Content-Type' : 'application/json',
            },
...
export const BuyCourse  = (id) => async(dispatch) => {
    try {
        ...
        const {data} = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/payment/${id}`, 
        {
            headers:{
                'Content-Type' : 'application/json',
            },
...
export const PaymentVerification = (id) => async(dispatch) => {
    try {
            ...
            const {data} = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/paymentverification/${id}`, 
            {
              headers:{
                 'Content-Type' : 'application/json',
             },
